...
import { admins, renewals, renewalNotifications } from "@/server/db/schema";
...
//     .select({
//       renewalId: renewalNotifications.renewalId,
//       lastNotificationSentAt: max(renewalNotifications.sentAt),
//     })
//     .from(renewalNotifications)
//     .where(eq(renewalNotifications.status, "SENT"))
//     .groupBy(renewalNotifications.renewalId)
...
export async function getRenewals({
  ...
  /* ------------------------------------------------------------------------ */
  /* Latest Notification Per Renewal                                         */
  /* ------------------------------------------------------------------------ */

  const latestNotifications = db
    .select({
      renewalId: renewalNotifications.renewalId,
      lastNotificationSentAt: max(renewalNotifications.sentAt).as(
  ...
  const [data, totalResult] = await Promise.all([
    db
      .select({
        ...
        lastNotificationSentAt: latestNotifications.lastNotificationSentAt,
      })
      .from(renewals)
      .innerJoin(admins, eq(renewals.adminId, admins.id))
      .leftJoin(
        latestNotifications,
        eq(renewals.id, latestNotifications.renewalId),
      )
      .where(whereClause)
      .orderBy(desc(renewals.dueDate))
      .limit(currentLimit)
